...
const MasterForm = () => {
  ...
  const handleSave = () => {
    ...
    const newEntry = { name: form.name.trim(), code: form.code.trim().toUpperCase(), status: "Active", createdAt: today, updatedAt: today };

    const existing = JSON.parse(localStorage.getItem(getMasterKey(type))) || [];
    localStorage.setItem(getMasterKey(type), JSON.stringify([...existing, newEntry]));
