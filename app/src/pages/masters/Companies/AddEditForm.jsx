...
const AddEditForm = ({ onSubmit, initialData = {} }) => {
  ...
  const handleSubmit = () => {
    if (formData.companyCode.trim() === '') {
      ...
      return;
    } else if (formData.companyDesc.trim() === '') {
      dispatch(showSnackbar({ message: 'Description is required !', severity: 'error' }));
      return;
    }
  ...
  return (
    <>
      ...
      >
        ...
        >
          {[
            { name: 'companyCode', label: 'Code' },
            { name: 'companyDesc', label: 'Description' }
          ].map((name) => (
            <TextField
              key={name.name}
              name={name.name}
