...
const AccountType = () => {
  ...
  const getDta = React.useCallback(async () => {
    try {
      const res = await axiosInstance.get(`/api/acctyp/fetch`)
      const dta = res.data
      if (res.status === 200) {
  ...
  const handleAddNew = async (formData) => {
    try {
      const res = await axiosInstance.post(`/api/acctyp/create`, formData)
      const dta = res.data
      if (res.status === 201) {
  ...
  const handleEditSave = async (formData) => {
    try {
      const res = await axiosInstance.patch(`/api/acctyp/update?id=${editData._id}`, formData);
      const dta = res.data;
  ...
  const handleEditCellChange = async (updatedRow, originalRow) => {
    try {
      ...
      const payLoad = {
        policytypeCode: updatedRow.policytypeCode,
        policytypeName: updatedRow.policytypeName,
      };

      if (payLoad.policytypeCode || payLoad.policytypeName) {
        payLoad.status = 'Active';
        // console.log(payLoad);

        const res = await axiosInstance.patch(`/api/acctyp/update?id=${updatedRow.id}`, payLoad);
        const dta = res.data;
  ...
  // 🔹 Toggle Level Status
  const handleLevelToggle = async (row) => {
    if (!window.confirm(`Change Same Level status of "${row.typname}" to "${!row.stacklvl}" ?`)) return;

    try {
        const res = await axiosInstance.patch(`/api/acctyp/update?id=${row._id}`, { stacklvl: !row.stacklvl})
        const dta = res.data
        if (res.status === 201) {
            dispatch(showSnackbar({ message: dta.message, severity: 'success' }))
            getDta()
  ...
  const handleStatusToggle = async (row) => {
    ...
    try {
      if (["Active","Inactive"].includes(newStatus)) {
        if (!window.confirm(`Change status of "${row.policytypeName}" to "${newStatus}" ?`)) return;
        const res = await axiosInstance.patch(`/api/acctyp/update?id=${row._id}`, { status: newStatus })
        const dta = res.data
        if (res.status === 201) {
          dispatch(showSnackbar({ message: dta.message, severity: 'success' }))
          getDta()
  ...
  const handleRemove = async () => {
    ...
    try {
      for (let rowId of idsToDelete) {
        ...
        if (row?._id) {
          await axiosInstance.delete(`/api/acctyp/delete?id=${row._id}`);
        }
      }
  ...
  const dataFields = [
    ...
    { field: 'typname', headerName: 'Name', width: 350 },
    { field: 'heirarchy', headerName: 'Heirarchy', width: 200 },
    { field: 'stacklvl', headerName: 'Same Level', width: 200, renderCell: (params) => (
        <span onClick={() => handleLevelToggle(params.row)}>{String(params.row.stacklvl)}</span>
    ) },
    { 
  ...
  return (
    <div className='main-body'>
      <div className="main-hdr">
        <span className='hds'>Account Type Details</span>
      </div>
      ...
      <div className="main-dtl">
        ...
        >
          <Toolbar sx={{
            ...
            <DialogTitle sx={{ p: 0 }}>
              {editData ? 'Edit Account Type' : 'Add New Account Type'}
            </DialogTitle>
