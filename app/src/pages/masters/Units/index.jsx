...
const Unit = () => {
  ...
  const getDta = React.useCallback(async () => {
    try {
      const res = await axiosInstance.get(`/api/unt/fetch`)
      const dta = res.data
      if (res.status === 200) {
  ...
  const handleAddNew = async (formData) => {
    try {
      const res = await axiosInstance.post(`/api/unt/create`, formData)
      const dta = res.data
      if (res.status === 201) {
  ...
  const handleEditSave = async (formData) => {
    try {
      const res = await axiosInstance.put(`/api/unt/update?id=${editData._id}`, formData);
      const dta = res.data;
  ...
  const handleEditCellChange = async (updatedRow, originalRow) => {
    try {
      ...
      const payLoad = {
        unitCode: updatedRow.unitCode,
        unitName: updatedRow.unitName,
      };

      if (payLoad.unitCode || payLoad.unitName) {
        payLoad.status = 'Active';
        // console.log(payLoad);

        const res = await axiosInstance.put(`/api/unt/update?id=${updatedRow.id}`, payLoad);
        const dta = res.data;
  ...
  const handleStatusToggle = async (row) => {
    ...
    try {
      if (["Active","Inactive"].includes(newStatus)) {
        if (!window.confirm(`Change status of "${row.unitCode}" to "${newStatus}" ?`)) return;
        const res = await axiosInstance.put(`/api/unt/update?id=${row._id}`, { status: newStatus })
        const dta = res.data
        if (res.status === 201) {
          dispatch(showSnackbar({ message: dta.message, severity: 'success' }))
          getDta()
  ...
  const handleRemove = async () => {
    ...
    try {
      for (let rowId of idsToDelete) {
        ...
        if (row?._id) {
          await axiosInstance.delete(`/api/unt/delete?id=${row._id}`);
        }
      }
  ...
  const dataFields = [
    ...
    { field: 'unitCode', headerName: 'Code', width: 350, editable: true },
    { field: 'unitName', headerName: 'Name', width: 350, editable: true },
    { 
      field: 'status', 
      headerName: 'Status', 
      width: 120,
  ...
  return (
    <div className='main-body'>
      <div className="main-hdr">
        <span className='hds'>Unit Details</span>
      </div>
