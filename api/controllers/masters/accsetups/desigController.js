import desigModel from '../../../models/masters/accsetups/desigModel.js';
...
const create = async (req, res) => {
    try {
        ...
        if (existingDesig) {
            ...
            if (!desig) {
                return res.status(401).json({ message: "Failed to create Designation record" });
            } else {
                res.status(201).json({
...
const update = async (req, res) => {
    try {
        ...
        Object.assign(desigPayld, { status: 'Active', updatedby: user._id });
